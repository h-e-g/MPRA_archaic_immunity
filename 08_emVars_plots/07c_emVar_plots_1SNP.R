...
dir.create(sprintf("%s/emVar_plots/custom", FIGURE_EMVAR_DIR), showWarnings = FALSE, recursive = TRUE)

tic("plotting emVars")
for (myPOSID in emVARs_obs[, unique(posID)]) {
  print(myPOSID)
  cat("\n", myPOSID, "\n")
  ...
  # all conditions
  pdf(sprintf("%s/emVar_plots/all_conditions/variant_%s_01a_SNPeffect.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 4)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "all", highlight_conditions = highlight_conds)
  try(print(p))
  dev.off()

  pdf(sprintf("%s/emVar_plots/all_conditions/variant_%s_01b_SNPeffect_reps.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 4)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "replicates", highlight_conditions = highlight_conds)
  try(print(p))
  dev.off()
  # all NS
  pdf(sprintf("%s/emVar_plots/NS_only/variant_%s_02a_SNPeffect.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.6)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "all", highlight_conditions = highlight_conds, target_conditions = "NS")
  try(print(p))
  dev.off()

  pdf(sprintf("%s/emVar_plots/NS_only/variant_%s_02b_SNPeffect_reps.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.6)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "replicates", highlight_conditions = highlight_conds, target_conditions = "NS")
  try(print(p))
  dev.off()

  # HepG2
  pdf(sprintf("%s/emVar_plots/HepG2_only/variant_%s_02a_SNPeffect.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.8)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "all", highlight_conditions = highlight_conds, target_cellines = "HepG2")
  try(print(p))
  dev.off()

  pdf(sprintf("%s/emVar_plots/HepG2_only/variant_%s_02b_SNPeffect_reps.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.8)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "replicates", highlight_conditions = highlight_conds, target_cellines = "HepG2")
  try(print(p))
  dev.off()

  # A549
  pdf(sprintf("%s/emVar_plots/A549_only/variant_%s_02a_SNPeffect.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.8)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "all", highlight_conditions = highlight_conds, target_cellines = "A549")
  try(print(p))
  dev.off()


  pdf(sprintf("%s/emVar_plots/A549_only/variant_%s_02b_SNPeffect_reps.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.8)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "replicates", highlight_conditions = highlight_conds, target_cellines = "A549")
  try(print(p))
  dev.off()

  # K562
  pdf(sprintf("%s/emVar_plots/K562_only/variant_%s_02a_SNPeffect.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.8)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "all", highlight_conditions = highlight_conds, target_cellines = "K562")
  try(print(p))
  dev.off()

  pdf(sprintf("%s/emVar_plots/K562_only/variant_%s_02b_SNPeffect_reps.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 2, width = 1.8)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "replicates", highlight_conditions = highlight_conds, target_cellines = "K562")
  try(print(p))
  dev.off()
}
toc()
...
rsID_list=c('rs1023693810','rs4766674','rs34016964','rs1510475')
for (myRSID in rsID_list){
  
  myPOSID=SNP_annot[rsID==myRSID,posID]
  print(myPOSID)
  cat("\n", myPOSID, "\n")
  #myRSID <- try(SNP_annot[posID == myPOSID, get("rsID")])
  highlight_conds <- emVARs_obs[posID == myPOSID, unique(COND_ID)]
  ...
  pdf(sprintf("%s/emVar_plots/custom/variant_%s_02a_SNPeffect.pdf", FIGURE_EMVAR_DIR, rsID_print), height = 1.8, width = 2)
  p <- plot_SNP(POSID = myPOSID, GC_correct = TRUE, select = "all", highlight_conditions = highlight_conds, target_conditions = "NS")
  try(print(p))
  dev.off()
}
